...
def detect_latest_week_tag() -> str:
    ...
    for p in Path("phase5_delivery/outputs").glob("combined_payload_*.json"):
        candidates.append(p)
    for p in Path("phase5_delivery/outputs").glob("doc_append_report_*.json"):
        candidates.append(p)
    ...
    m = re.search(r"(?:combined_payload|doc_append_report|pulse)_(.+)\.\w+$", name)
    return m.group(1) if m else current_week_tag()
...
def check_doc_append(doc_report: dict[str, Any]) -> GateResult:
    doc_status = str(doc_report.get("status", "")).lower()
    ok = doc_status == "pass"
    return GateResult(
        gate="doc_append_status",
        status="pass" if ok else "fail",
        details={
...
def build_run_summary(*, week: str, paths: dict[str, str]) -> dict[str, Any]:
    ...
    gates.append(check_doc_append(doc_report))
    ...
    return {
        ...
        "high_level_metrics": {
            ...
            "doc_append_status": doc_report.get("status"),
        },
        ...
        "upstream_status": {
            ...
            "phase5_doc": doc_report.get("status"),
        },
    }
